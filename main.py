...
def generate_spell():
    ...
    output_field.insert('1.0',f'/give @p minecraft:leather_horse_armor{{\n'
                    ...
                    f'    hexenwerk-lore_display: \'{{"text":"\u2022 {spell_name}","color":"{spell_color}","italic":false}}\',\n'
                    f'    hexenwerk-chat_display: \'{{"text":"{spell_name}","color":"{spell_color}","italic":false}}\',\n'
...
input_start_raycast_text.grid(row=8, column=0, sticky="w", pady=(10, 0))

input_start_raycast = ttk.Entry(master=inputs_frame, font="Noto-Sans-Lisu 15")
input_start_raycast.grid(row=9, column=0, pady=2, padx=4, sticky="w")
...
input_block_collision_text.grid(row=10, column=0, sticky="w", pady=(10, 0))

input_block_collision = ttk.Entry(master=inputs_frame, font="Noto-Sans-Lisu 15")
input_block_collision.grid(row=11, column=0, pady=2, padx=4, sticky="w")
...
input_entity_collision_as_player_text.grid(row=12, column=0, sticky="w", pady=(10, 0))

input_entity_collision_as_player = ttk.Entry(master=inputs_frame, font="Noto-Sans-Lisu 15")
input_entity_collision_as_player.grid(row=13, column=0, pady=2, padx=4, sticky="w")
...
input_entity_collision_as_target_text.grid(row=14, column=0, sticky="w", pady=(10, 0))

input_entity_collision_as_target = ttk.Entry(master=inputs_frame, font="Noto-Sans-Lisu 15")
input_entity_collision_as_target.grid(row=15, column=0, pady=2, padx=4, sticky="w")

## on cast
input_on_cast_text = ttk.Label(master=inputs_frame, text="Cast Command", font="Noto-Sans-Lisu 17", background='#D2CBF2')
input_on_cast_text.grid(row=16, column=0, sticky="w", pady=(10, 0))

input_on_cast = ttk.Entry(master=inputs_frame, font="Noto-Sans-Lisu 15")
input_on_cast.grid(row=17, column=0, pady=2, padx=4, sticky="w")
...
input_on_raycast_step_text.grid(row=18, column=0, sticky="w", pady=(10, 0))

input_on_raycast_step = ttk.Entry(master=inputs_frame, font="Noto-Sans-Lisu 15")
input_on_raycast_step.grid(row=19, column=0, pady=2, padx=4, sticky="w")
